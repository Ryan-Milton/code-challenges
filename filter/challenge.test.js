...
const filterStringsWithVowels = (input) => {
  // Solution code here...
  let filteredString = input.filter(vowel => {
...
const notInFirstArray = (forbiddenValues, input) => {
  // Solution code here...
  let manyArr = input.filter((pri) => {
      return (pri.length > forbiddenValues.length);
  })
  return manyArr;
};
...
const getBaseStatGreaterThan = (input, minBaseStat) => {
  // Solution code here...
  let stat = input.filter( (snorlax) => snorlax.baseStat > minBaseStat);
  return stat;
};
...
const getStatName = (input, minBaseStat) => {
  // Solution code here...
  let anotherStat = input.filter( (snorlax) => {
    if (snorlax.baseStat > minBaseStat) {
      return Object.getOwnPropertyNames(snorlax.baseStat.name);
    }
  })
  console.log(anotherStat);
  return anotherStat;
};
